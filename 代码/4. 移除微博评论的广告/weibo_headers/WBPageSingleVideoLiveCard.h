//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "WBPageCard.h"

@class NSString, WBPageSingleVideoLiveConfig, WBStatus;

@interface WBPageSingleVideoLiveCard : WBPageCard
{
    _Bool _isTopProfileInfoHidden;
    WBStatus *_status;
    NSString *_bottomLiveTitle;
    NSString *_bottomLiveTitleScheme;
    NSString *_desc1;
    WBPageSingleVideoLiveConfig *_businessConfig;
}

+ (Class)viewClass;
@property(retain, nonatomic) WBPageSingleVideoLiveConfig *businessConfig; // @synthesize businessConfig=_businessConfig;
@property(nonatomic) _Bool isTopProfileInfoHidden; // @synthesize isTopProfileInfoHidden=_isTopProfileInfoHidden;
@property(retain, nonatomic) NSString *desc1; // @synthesize desc1=_desc1;
@property(retain, nonatomic) NSString *bottomLiveTitleScheme; // @synthesize bottomLiveTitleScheme=_bottomLiveTitleScheme;
@property(retain, nonatomic) NSString *bottomLiveTitle; // @synthesize bottomLiveTitle=_bottomLiveTitle;
@property(retain, nonatomic) WBStatus *status; // @synthesize status=_status;
- (void).cxx_destruct;
- (_Bool)updateWithDictionary:(id)arg1;

@end

