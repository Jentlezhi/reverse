//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@class NSData, NSDictionary, NSMutableData, NSString;

@interface WBAudioHTTPRequest : NSObject
{
    CDUnknownBlockType _completedBlock;
    CDUnknownBlockType _progressBlock;
    CDUnknownBlockType _didReceiveResponseBlock;
    CDUnknownBlockType _didReceiveDataBlock;
    NSString *_userAgent;
    double _timeoutInterval;
    struct __CFHTTPMessage *_message;
    struct __CFReadStream *_responseStream;
    NSDictionary *_responseHeaders;
    NSMutableData *_responseData;
    NSString *_responseString;
    long long _statusCode;
    NSString *_statusMessage;
    _Bool _failed;
    double _startedTime;
    unsigned long long _downloadSpeed;
    unsigned long long _responseContentLength;
    unsigned long long _receivedLength;
}

+ (id)defaultUserAgent;
+ (double)defaultTimeoutInterval;
+ (id)requestWithURL:(id)arg1;
@property(copy) CDUnknownBlockType didReceiveDataBlock; // @synthesize didReceiveDataBlock=_didReceiveDataBlock;
@property(copy) CDUnknownBlockType didReceiveResponseBlock; // @synthesize didReceiveResponseBlock=_didReceiveResponseBlock;
@property(copy) CDUnknownBlockType progressBlock; // @synthesize progressBlock=_progressBlock;
@property(copy) CDUnknownBlockType completedBlock; // @synthesize completedBlock=_completedBlock;
@property(readonly, nonatomic, getter=isFailed) _Bool failed; // @synthesize failed=_failed;
@property(readonly, nonatomic) unsigned long long downloadSpeed; // @synthesize downloadSpeed=_downloadSpeed;
@property(readonly, nonatomic) NSString *statusMessage; // @synthesize statusMessage=_statusMessage;
@property(readonly, nonatomic) long long statusCode; // @synthesize statusCode=_statusCode;
@property(readonly, nonatomic) unsigned long long responseContentLength; // @synthesize responseContentLength=_responseContentLength;
@property(readonly, nonatomic) NSDictionary *responseHeaders; // @synthesize responseHeaders=_responseHeaders;
@property(readonly, nonatomic) NSData *responseData; // @synthesize responseData=_responseData;
@property(retain, nonatomic) NSString *userAgent; // @synthesize userAgent=_userAgent;
@property(nonatomic) double timeoutInterval; // @synthesize timeoutInterval=_timeoutInterval;
- (void).cxx_destruct;
- (void)p_responseStreamErrorOccurred;
- (void)p_responseStrameEndEncountered;
- (void)p_responseStreamHasBytesAvailable;
- (void)p_closeResponseStream;
- (void)p_updateDownloadSpeed;
- (void)p_updateProgress;
- (void)p_readResponseHeaders;
- (void)p_checkResponseContentLength;
- (void)p_invokeDidReceiveDataBlockWithData:(id)arg1;
- (void)p_invokeDidReceiveResponseBlock;
- (void)p_invokeProgressBlockWithDownloadProgress:(double)arg1;
- (void)p_invokeCompletedBlock;
- (void)dealloc;
@property(readonly, nonatomic) NSString *responseString;
- (void)cancel;
- (void)start;
- (id)initWithURL:(id)arg1;

@end

