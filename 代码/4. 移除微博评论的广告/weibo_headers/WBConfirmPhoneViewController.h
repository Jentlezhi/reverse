//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "WBViewController.h"

#import "WBLoginManagerProtocol-Protocol.h"

@class NSString, UILabel, WBStyleButton;

@interface WBConfirmPhoneViewController : WBViewController <WBLoginManagerProtocol>
{
    _Bool isInLoginManager;
    NSString *_encryptedPhoneNumber;
    NSString *_accountName;
    UILabel *_messageLabel;
    WBStyleButton *_confirmButton;
    WBStyleButton *_rejectButton;
}

@property(retain, nonatomic) WBStyleButton *rejectButton; // @synthesize rejectButton=_rejectButton;
@property(retain, nonatomic) WBStyleButton *confirmButton; // @synthesize confirmButton=_confirmButton;
@property(retain, nonatomic) UILabel *messageLabel; // @synthesize messageLabel=_messageLabel;
@property(retain, nonatomic) NSString *accountName; // @synthesize accountName=_accountName;
@property(retain, nonatomic) NSString *encryptedPhoneNumber; // @synthesize encryptedPhoneNumber=_encryptedPhoneNumber;
@property(nonatomic) _Bool isInLoginManager; // @synthesize isInLoginManager;
- (void).cxx_destruct;
- (void)showHudWithError:(id)arg1;
- (void)openForgetPasswordWebsite;
- (void)rejectButtonPressed;
- (void)confirmButtonPressed;
- (void)configSubviewsFrame;
- (void)setupSubviews;
- (void)viewDidLoad;
- (void)updatePropertyWithPara:(id)arg1 context:(id)arg2 currentStep:(unsigned long long)arg3;
- (id)moduleID;
- (id)init;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

