//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "WBMModule.h"

@class NSDictionary;

@interface WBFeedCoreModule : WBMModule
{
    NSDictionary *_forceGroupDict;
}

+ (id)debugPageItem;
@property(retain, nonatomic) NSDictionary *forceGroupDict; // @synthesize forceGroupDict=_forceGroupDict;
- (void).cxx_destruct;
- (void)module:(id)arg1 pushDailyDidChanged:(id)arg2;
- (void)module:(id)arg1 pushActiveDidChanged:(id)arg2;
- (void)module:(id)arg1 registCube:(id)arg2;
- (void)module:(id)arg1 didUpgradeFromVersion:(int)arg2 toVersion:(int)arg3;
- (void)module:(id)arg1 didFinishSetupWithOptions:(id)arg2;
- (void)module:(id)arg1 setupWithOptions:(id)arg2;
- (void)updateDB:(id)arg1;
- (void)wbm_main;
- (void)wbm_pre_main;

@end

