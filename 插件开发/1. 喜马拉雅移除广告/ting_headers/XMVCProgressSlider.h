//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <UIKit/UIControl.h>

#import "UIGestureRecognizerDelegate-Protocol.h"

@class NSString, UIColor, UIImage;

@interface XMVCProgressSlider : UIControl <UIGestureRecognizerDelegate>
{
    struct CGRect _dotRect;
    double _width;
    double _offsetX;
    _Bool _isTracking;
    _Bool _disabled;
    double _value;
    double _lineHeight;
    double _dotDiameter;
    UIImage *_dotImage;
    UIColor *_minColor;
    UIColor *_dotColor;
    UIColor *_maxColor;
    double _margin;
}

@property(nonatomic) double margin; // @synthesize margin=_margin;
@property(retain, nonatomic) UIColor *maxColor; // @synthesize maxColor=_maxColor;
@property(retain, nonatomic) UIColor *dotColor; // @synthesize dotColor=_dotColor;
@property(retain, nonatomic) UIColor *minColor; // @synthesize minColor=_minColor;
@property(retain, nonatomic) UIImage *dotImage; // @synthesize dotImage=_dotImage;
@property(nonatomic) double dotDiameter; // @synthesize dotDiameter=_dotDiameter;
@property(nonatomic) double lineHeight; // @synthesize lineHeight=_lineHeight;
@property(nonatomic) _Bool disabled; // @synthesize disabled=_disabled;
@property(nonatomic) double value; // @synthesize value=_value;
- (void).cxx_destruct;
- (void)onPan:(id)arg1;
- (struct CGRect)touchReferRect;
- (void)onTap:(id)arg1;
- (_Bool)gestureRecognizerShouldBegin:(id)arg1;
- (_Bool)_interalSetValue:(double)arg1;
- (struct CGSize)dotSize;
@property(readonly, nonatomic) struct CGPoint dotCenter;
- (_Bool)isTracking;
- (_Bool)tracking;
- (struct CGSize)sizeThatFits:(struct CGSize)arg1;
- (void)drawRect:(struct CGRect)arg1;
- (double)width;
- (struct CGSize)touchDotSize;
- (void)baseInit;
- (id)initWithFrame:(struct CGRect)arg1;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

