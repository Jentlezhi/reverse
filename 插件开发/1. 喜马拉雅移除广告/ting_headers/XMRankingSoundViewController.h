//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "XMSubLevelViewController.h"

#import "XMFocusImageViewDelegate-Protocol.h"
#import "XMSoundTableViewCellDelegate-Protocol.h"
#import "XMVXPageViewSubControllerProtocol-Protocol.h"

@class NSMutableArray, NSString, XMCategoryItem, XMFocusImageView, XMRankCategoryItem, XMRankItem, XMServerRequest;

@interface XMRankingSoundViewController : XMSubLevelViewController <XMSoundTableViewCellDelegate, XMFocusImageViewDelegate, XMVXPageViewSubControllerProtocol>
{
    long long _pageId;
    long long _maxPageId;
    XMRankItem *_rank;
    XMRankCategoryItem *_catgoryItem;
    XMCategoryItem *_category;
    XMServerRequest *_rankRequest;
    NSMutableArray *_rankDatas;
    XMFocusImageView *_focusImageView;
}

@property(retain, nonatomic) XMFocusImageView *focusImageView; // @synthesize focusImageView=_focusImageView;
@property(retain, nonatomic) NSMutableArray *rankDatas; // @synthesize rankDatas=_rankDatas;
@property(retain, nonatomic) XMServerRequest *rankRequest; // @synthesize rankRequest=_rankRequest;
@property(retain, nonatomic) XMCategoryItem *category; // @synthesize category=_category;
@property(retain, nonatomic) XMRankCategoryItem *catgoryItem; // @synthesize catgoryItem=_catgoryItem;
@property(retain, nonatomic) XMRankItem *rank; // @synthesize rank=_rank;
- (void).cxx_destruct;
- (id)vxContentScrollView;
- (void)postOpenSoundItemDataTrackForPolymericRank:(long long)arg1 sound:(id)arg2;
- (void)postOpenSoundItemDataTrack:(long long)arg1 sound:(id)arg2;
- (_Bool)shouldTriggerBackPan:(id)arg1;
- (void)focusImageView:(id)arg1 clickFocusItem:(id)arg2;
- (void)addFocusImages:(id)arg1;
- (void)addFocusImageView;
- (void)reloadDataFromDownloadNotification:(id)arg1;
- (void)onDownloadSuccess:(id)arg1;
- (void)onDownloadFailed:(id)arg1;
- (void)onDownloadStart:(id)arg1;
- (void)onCurrentTrackEnd;
- (void)onPlayingTrackPaused;
- (void)onPlayingTrackChanged;
- (void)downloadSound:(id)arg1;
- (void)playerSound:(id)arg1;
- (void)tableView:(id)arg1 didSelectRowAtIndexPath:(id)arg2;
- (double)tableView:(id)arg1 heightForRowAtIndexPath:(id)arg2;
- (id)tableView:(id)arg1 cellForRowAtIndexPath:(id)arg2;
- (long long)tableView:(id)arg1 numberOfRowsInSection:(long long)arg2;
- (void)reloadData;
- (void)triggerLoadMore;
- (void)triggerPullToRefresh;
- (_Bool)isLoadMoreViewNeeded;
- (void)onFailed:(id)arg1;
- (void)onFinished:(id)arg1;
- (void)requestRankData:(long long)arg1;
- (void)didReceiveMemoryWarning;
- (void)viewDidDisappear:(_Bool)arg1;
- (void)viewWillAppear:(_Bool)arg1;
- (void)viewDidAppear:(_Bool)arg1;
- (void)viewDidLoad;
- (id)init;
- (void)dealloc;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

