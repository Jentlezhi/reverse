//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "XMSubLevelViewController.h"

#import "XMAlbumCellDelegate-Protocol.h"

@class NSMutableArray, NSString, XMCategoryItem, XMKeywordItem, XMServerRequest;

@interface XMCategorySubfieldsViewController : XMSubLevelViewController <XMAlbumCellDelegate>
{
    long long _pageId;
    long long _maxPageId;
    XMCategoryItem *_categoryItem;
    XMKeywordItem *_keywordItem;
    NSString *_categoryTag;
    NSString *_subfield;
    XMServerRequest *_listRequest;
    NSMutableArray *_listDatas;
}

@property(retain, nonatomic) NSMutableArray *listDatas; // @synthesize listDatas=_listDatas;
@property(retain, nonatomic) XMServerRequest *listRequest; // @synthesize listRequest=_listRequest;
@property(copy, nonatomic) NSString *subfield; // @synthesize subfield=_subfield;
@property(copy, nonatomic) NSString *categoryTag; // @synthesize categoryTag=_categoryTag;
@property(retain, nonatomic) XMKeywordItem *keywordItem; // @synthesize keywordItem=_keywordItem;
@property(retain, nonatomic) XMCategoryItem *categoryItem; // @synthesize categoryItem=_categoryItem;
- (void).cxx_destruct;
- (double)tableView:(id)arg1 heightForRowAtIndexPath:(id)arg2;
- (void)tableView:(id)arg1 didSelectRowAtIndexPath:(id)arg2;
- (id)tableView:(id)arg1 cellForRowAtIndexPath:(id)arg2;
- (long long)tableView:(id)arg1 numberOfRowsInSection:(long long)arg2;
- (void)requestDataFinish;
- (void)onReqestFailed:(id)arg1;
- (void)onRequestFinished:(id)arg1;
- (void)requestFromIndex:(long long)arg1;
- (void)triggerLoadMore;
- (_Bool)isLoadMoreViewNeeded;
- (void)triggerPullToRefresh;
- (void)didReceiveMemoryWarning;
- (void)viewDidDisappear:(_Bool)arg1;
- (void)viewDidAppear:(_Bool)arg1;
- (void)viewDidLoad;
- (id)initWithCategoryItem:(id)arg1 keywordItem:(id)arg2 subfieldName:(id)arg3;
- (void)dealloc;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

