//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <UIKit/UICollectionViewCell.h>

@class UIImageView, UILabel, XMMGradualCornerView, XMMImitateVideoModel, XMWebImageView;

@interface XMMImitateVideoCell : UICollectionViewCell
{
    XMMImitateVideoModel *_model;
    XMWebImageView *_coverImageView;
    UIImageView *_playCountImage;
    UILabel *_playCountLabel;
    XMMGradualCornerView *_gradualView;
}

@property(retain, nonatomic) XMMGradualCornerView *gradualView; // @synthesize gradualView=_gradualView;
@property(retain, nonatomic) UILabel *playCountLabel; // @synthesize playCountLabel=_playCountLabel;
@property(retain, nonatomic) UIImageView *playCountImage; // @synthesize playCountImage=_playCountImage;
@property(retain, nonatomic) XMWebImageView *coverImageView; // @synthesize coverImageView=_coverImageView;
@property(retain, nonatomic) XMMImitateVideoModel *model; // @synthesize model=_model;
- (void).cxx_destruct;
- (void)didSetData;
- (void)customInit;
- (id)initWithFrame:(struct CGRect)arg1;

@end

