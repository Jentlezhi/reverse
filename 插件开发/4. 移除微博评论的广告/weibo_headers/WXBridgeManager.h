//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@class WXBridgeContext, WXSDKInstance, WXThreadSafeMutableArray;

@interface WXBridgeManager : NSObject
{
    _Bool _stopRunning;
    WXBridgeContext *_bridgeCtx;
    WXThreadSafeMutableArray *_instanceIdStack;
}

+ (void)_performBlockSyncOnBridgeThread:(CDUnknownBlockType)arg1;
+ (void)_performBlockOnBridgeThread:(CDUnknownBlockType)arg1;
+ (id)jsThread;
+ (id)sharedManager;
@property(retain, nonatomic) WXThreadSafeMutableArray *instanceIdStack; // @synthesize instanceIdStack=_instanceIdStack;
@property(retain, nonatomic) WXBridgeContext *bridgeCtx; // @synthesize bridgeCtx=_bridgeCtx;
@property(nonatomic) _Bool stopRunning; // @synthesize stopRunning=_stopRunning;
- (void).cxx_destruct;
- (void)executeJsMethod:(id)arg1;
- (void)callJSMethod:(id)arg1 args:(id)arg2;
- (void)resetEnvironment;
- (void)logToWebSocket:(id)arg1 message:(id)arg2;
- (void)connectToWebSocket:(id)arg1;
- (void)connectToDevToolWithUrl:(id)arg1;
- (void)callBack:(id)arg1 funcId:(id)arg2 params:(id)arg3;
- (void)callBack:(id)arg1 funcId:(id)arg2 params:(id)arg3 keepAlive:(_Bool)arg4;
- (id)fireEventWithResult:(id)arg1 ref:(id)arg2 type:(id)arg3 params:(id)arg4 domChanges:(id)arg5;
- (void)callComponentHook:(id)arg1 componentId:(id)arg2 type:(id)arg3 hook:(id)arg4 args:(id)arg5 competion:(CDUnknownBlockType)arg6;
- (void)fireEvent:(id)arg1 ref:(id)arg2 type:(id)arg3 params:(id)arg4 domChanges:(id)arg5 handlerArguments:(id)arg6;
- (void)fireEvent:(id)arg1 ref:(id)arg2 type:(id)arg3 params:(id)arg4 domChanges:(id)arg5;
- (void)fireEvent:(id)arg1 ref:(id)arg2 type:(id)arg3 params:(id)arg4;
- (void)registerComponents:(id)arg1;
- (void)registerModules:(id)arg1;
- (void)unregisterService:(id)arg1;
- (void)registerService:(id)arg1 withService:(id)arg2 withOptions:(id)arg3 completion:(CDUnknownBlockType)arg4;
- (void)registerService:(id)arg1 withServiceUrl:(id)arg2 withOptions:(id)arg3 completion:(CDUnknownBlockType)arg4;
- (void)DownloadJS:(id)arg1 url:(id)arg2 completion:(CDUnknownBlockType)arg3;
- (id)callJSMethodWithResult:(id)arg1;
- (void)callJsMethod:(id)arg1;
- (void)executeJsFramework:(id)arg1;
- (void)updateState:(id)arg1 data:(id)arg2;
- (void)refreshInstance:(id)arg1 data:(id)arg2;
- (void)forceGarbageCollection;
- (void)destroyInstance:(id)arg1;
- (id)getInstanceIdStack;
- (void)createInstance:(id)arg1 contents:(id)arg2 options:(id)arg3 data:(id)arg4;
- (void)createInstance:(id)arg1 template:(id)arg2 options:(id)arg3 data:(id)arg4;
- (void)createInstanceForJS:(id)arg1 template:(id)arg2 options:(id)arg3 data:(id)arg4;
- (void)_runLoopThread;
- (void)unload;
@property(readonly, nonatomic) __weak WXSDKInstance *topInstance;
- (id)init;

@end

