//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "NSObject-Protocol.h"

@class NSDate, NSError, NSString;

@protocol WBVideoPlaybackSessionMetricsEvent <NSObject>
@property(readonly, nonatomic) unsigned long long numberOfStalls;
@property(readonly, nonatomic) double durationWatched;
@property(readonly, copy, nonatomic) NSString *URI;
@property(readonly, nonatomic) NSError *error;
@property(readonly, nonatomic) NSDate *playbackReadyDate;
@property(readonly, nonatomic) NSDate *playbackStartDate;
@end

