//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "WBUniversalCardListViewController.h"

#import "WBBaseSegmentListViewWrapperDelegate-Protocol.h"
#import "WBComposeViewControllerDelegate-Protocol.h"
#import "WBConfigToobarMenuProtocol-Protocol.h"
#import "WBSegmentCardListViewControllerCallback-Protocol.h"

@class NSMutableDictionary, NSString, QRCodeEncodeEngine, UIBarButtonItem, WBCardListHeaderChannelCard, WBCardListViewWrapper;
@protocol WBSegmentCardListViewControllerCallback;

@interface WBCardListViewController : WBUniversalCardListViewController <WBBaseSegmentListViewWrapperDelegate, WBComposeViewControllerDelegate, WBSegmentCardListViewControllerCallback, WBConfigToobarMenuProtocol>
{
    _Bool _notAllowSegmentMode;
    _Bool _isFullScreenMode;
    _Bool _isSegmentViewMode;
    _Bool _needCheckLocationPerAndUpload;
    _Bool _backItemDidHide;
    _Bool _isFirstLoad;
    _Bool _disableAutoLoadmore;
    id <WBSegmentCardListViewControllerCallback> _callback;
    WBCardListViewWrapper *_wrapper;
    NSString *_requestApiPath;
    QRCodeEncodeEngine *_codeEncodeEngine;
    WBCardListHeaderChannelCard *_channelCard;
    UIBarButtonItem *_storageBackItem;
    NSMutableDictionary *_segmentViewControllerInfos;
}

+ (_Bool)needRecordCardlistTimeForContainerid:(id)arg1;
@property(nonatomic) _Bool disableAutoLoadmore; // @synthesize disableAutoLoadmore=_disableAutoLoadmore;
@property(retain, nonatomic) NSMutableDictionary *segmentViewControllerInfos; // @synthesize segmentViewControllerInfos=_segmentViewControllerInfos;
@property(retain, nonatomic) UIBarButtonItem *storageBackItem; // @synthesize storageBackItem=_storageBackItem;
@property(nonatomic) _Bool isFirstLoad; // @synthesize isFirstLoad=_isFirstLoad;
@property(nonatomic) _Bool backItemDidHide; // @synthesize backItemDidHide=_backItemDidHide;
@property(retain, nonatomic) WBCardListHeaderChannelCard *channelCard; // @synthesize channelCard=_channelCard;
@property(retain, nonatomic) QRCodeEncodeEngine *codeEncodeEngine; // @synthesize codeEncodeEngine=_codeEncodeEngine;
@property(nonatomic) _Bool needCheckLocationPerAndUpload; // @synthesize needCheckLocationPerAndUpload=_needCheckLocationPerAndUpload;
@property(retain, nonatomic) NSString *requestApiPath; // @synthesize requestApiPath=_requestApiPath;
@property(retain, nonatomic) WBCardListViewWrapper *wrapper; // @synthesize wrapper=_wrapper;
@property(nonatomic) __weak id <WBSegmentCardListViewControllerCallback> callback; // @synthesize callback=_callback;
@property(nonatomic) _Bool isSegmentViewMode; // @synthesize isSegmentViewMode=_isSegmentViewMode;
- (void)setIsFullScreenMode:(_Bool)arg1;
@property(nonatomic) _Bool notAllowSegmentMode; // @synthesize notAllowSegmentMode=_notAllowSegmentMode;
- (void).cxx_destruct;
- (_Bool)generalMediaAutoPlayIgnored;
- (_Bool)panNavigationSimultaneouslyWithGestureRecognizer:(id)arg1;
- (_Bool)panNavigationShouldBegin:(id)arg1;
- (_Bool)panToNavigation;
- (void)handleContactUnFollow:(id)arg1;
- (id)filterInterruptCard:(id)arg1;
- (void)deleteInterruptCardInCardList;
- (void)markIfNeedForceReloadDataAndDeleteInterruptCard:(id)arg1;
- (void)deleteCardListInterruptAndLoadMore;
- (void)handleToolbarMenuFollow:(id)arg1;
- (void)commonButtonActionDidFinishedNotification:(id)arg1;
- (void)deletePageCardWithNotification:(id)arg1;
- (void)deleteTimeLineItemWithNotification:(id)arg1;
- (void)onReceivedShareData:(id)arg1 in:(id)arg2 containerId:(id)arg3;
- (void)onReceivedShareData:(id)arg1 in:(id)arg2;
- (id)cardListBaseItem;
- (void)onReceivedTitle:(id)arg1 in:(id)arg2 containerId:(id)arg3;
- (void)onReceivedTitle:(id)arg1 in:(id)arg2;
- (void)onReceivedFilterGroup:(id)arg1 in:(id)arg2 containerId:(id)arg3;
- (void)onReceivedFilterGroup:(id)arg1 in:(id)arg2;
- (_Bool)shouldConfigToobarMenu;
- (void)didPressedRefreshButtonWithContainerId:(id)arg1;
- (void)searchTypeMenu:(id)arg1 didSelectType:(id)arg2 forceReload:(_Bool)arg3;
- (void)searchTypeMenu:(id)arg1 didSelectType:(id)arg2;
- (void)didTriggerLoadMoreInWrapper:(id)arg1;
- (void)willEndPullRefreshInWrapper:(id)arg1;
- (void)willTriggerPullRefreshInWrapper:(id)arg1;
- (void)onReceivedBackwardInfo:(id)arg1;
- (void)setMenuButtons:(id)arg1;
- (void)setTitle:(id)arg1;
- (void)configFilterGroup:(id)arg1 atIndex:(unsigned long long)arg2;
- (void)configFilterGroup:(id)arg1;
@property(readonly, nonatomic) _Bool shouldConfigFilterGroupsWhenInSegmentMode;
- (id)currentCardListViewController;
- (id)currentTabContainerId;
- (void)updateTitleView;
- (id)allowedBackworkSchemes;
- (id)getCurrentIdsWithCurrentChannels:(id)arg1;
- (void)showBackItemButton;
- (void)hideBackItemButton;
- (void)refresh;
- (void)refreshCurrentListView;
- (void)sendChannelDataWithCurrentChannels:(id)arg1 unusedChannels:(id)arg2;
- (void)wrapperWillResetDefaultSelectedIndex:(long long)arg1;
- (void)wrapperDidUpdateChannels:(id)arg1 unusedChannels:(id)arg2;
- (void)wrapperDidChangeSwipeViewCurrentItem:(id)arg1;
- (_Bool)tableViewShouldBypassEmptyView:(id)arg1;
- (void)configCardListHeaderWithCard:(id)arg1 atIndex:(unsigned long long)arg2;
- (_Bool)supportPullRefrash;
- (void)switchToSegmentModeIfNeeded:(_Bool)arg1;
- (void)configData;
- (void)configTableViewStyle;
- (void)configTableViewInsets;
- (void)configSubviewsFrame;
- (_Bool)needLocation;
- (_Bool)isFullScreenMode;
- (void)updateChannelsData;
- (void)removeSubviewsIfNeeded;
- (Class)segmentViewWrapperClass;
- (id)analysisParameters;
- (void)initializeSubviewsIfNeeded;
@property(readonly, nonatomic) double navigationHeight;
- (void)initializeSelf;
- (void)rightNavigationItemPressed:(id)arg1;
- (void)scrollViewDidScroll:(id)arg1;
- (void)reloadUIElements;
- (void)currentSkinDidChangeNotification:(id)arg1;
- (void)sharePageViaStatusWithGroupType:(unsigned long long)arg1;
- (void)createAndShareQRCode;
- (void)unfollowArticle;
- (void)contactRecentPickerViewController:(id)arg1 shareToPrivateMessageWithUserTarget:(id)arg2;
- (id)apiPathForPageEngine:(id)arg1;
- (void)didTriggerPullRefreshInWrapper:(id)arg1;
- (void)didFinishPullRefreshing;
- (void)configViewsInCalling;
- (void)configToolbarMenu;
- (_Bool)locationInfoError;
- (void)showLocationPrivacyPromptIfNeeded;
- (void)shareCardlist;
- (void)viewDidDisappear:(_Bool)arg1;
- (void)markPollCardNeedRecaculateHeight:(id)arg1;
- (void)pollViewActionDidFinishNotification:(id)arg1;
- (void)updateTagDelete:(id)arg1;
- (void)updateTheBlogToPrivacy:(id)arg1;
- (void)updateSpecifiedPageCard:(id)arg1;
- (Class)baseViewClass;
- (void)viewDidLoad;
- (void)viewWillAppear:(_Bool)arg1;
- (id)initWithOpenUrlLink:(id)arg1 inAppParas:(struct NSMutableDictionary *)arg2;
- (void)dealloc;
- (struct CGRect)workingAreaRect;
- (id)workingAreaHandlers;
- (_Bool)workingAreaDetectionEnabled;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

