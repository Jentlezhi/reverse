//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "NSObject-Protocol.h"

@class NSArray, NSError, NSString;
@protocol WBFAVAsset, WBFAVMediaSelection, WBFAVMediaSelectionGroup, WBFAVMediaSelectionOption;

@protocol WBFAVPlayerItem <NSObject>
+ (NSString *)playbackDidReachEndTimeNotificationName;
+ (NSString *)playbackStalledNotificationName;
@property(nonatomic) double playbackMaximumBufferDuration;
@property(nonatomic) long long automaticMediaSelectionPreferedResolution;
@property(nonatomic) long long automaticMediaSelectionMaximumResolution;
@property(readonly, nonatomic) id <WBFAVMediaSelection> currentMediaSelection;
@property(readonly, nonatomic) _Bool cacheModuleActive;
@property(readonly, nonatomic) double startupTime;
@property(readonly, nonatomic) struct CGSize presentationSize;
@property(readonly, nonatomic, getter=isPlaybackLikelyToKeepUp) _Bool playbackLikelyToKeepUp;
@property(readonly) CDStruct_1b6d18a9 loadedTimeDuration;
@property(readonly, nonatomic) NSArray *loadedTimeRanges;
@property(readonly, nonatomic) CDStruct_1b6d18a9 duration;
@property(readonly, nonatomic) NSError *error;
@property(readonly, nonatomic) id <WBFAVAsset> asset;
@property(readonly, nonatomic) long long status;
- (void)selectMediaOptions:(NSArray *)arg1 automaticallyInMediaSelectionGroup:(id <WBFAVMediaSelectionGroup>)arg2;
- (void)selectMediaOptionAutomaticallyInMediaSelectionGroup:(id <WBFAVMediaSelectionGroup>)arg1;
- (void)selectMediaOption:(id <WBFAVMediaSelectionOption>)arg1 inMediaSelectionGroup:(id <WBFAVMediaSelectionGroup>)arg2;
- (void)seekToTime:(CDStruct_1b6d18a9)arg1 toleranceBefore:(CDStruct_1b6d18a9)arg2 toleranceAfter:(CDStruct_1b6d18a9)arg3 completionHandler:(void (^)(_Bool))arg4;
- (void)seekToTime:(CDStruct_1b6d18a9)arg1;
- (CDStruct_1b6d18a9)currentTime;
- (id)initWithAsset:(id <WBFAVAsset>)arg1;
@end

