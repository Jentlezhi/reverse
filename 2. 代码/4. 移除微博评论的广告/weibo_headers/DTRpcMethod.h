//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@class NSDictionary, NSString;
@protocol DTRpcPrivateEncryptProtocol;

@interface DTRpcMethod : NSObject
{
    _Bool _checkLogin;
    _Bool _signCheck;
    _Bool _isAliUserGWMethod;
    _Bool _retryable;
    _Bool _isIgnoreNetStatus;
    _Bool _isSpecialChannelRPC;
    _Bool _isNSData;
    _Bool _isLoginRPC;
    _Bool _httpGET;
    _Bool _isCrypt;
    _Bool _enableCrypt;
    _Bool _cookieReWrite;
    NSString *_operationType;
    Class _resultClass;
    NSString *_returnType;
    Class _elementClass;
    NSString *_apdid;
    long long _cacheStrategy;
    double _timeoutInterval;
    long long _logLevel;
    id <DTRpcPrivateEncryptProtocol> _privateEncrypt;
    NSDictionary *_extInfo;
    NSString *_productId;
}

@property(retain, nonatomic) NSString *productId; // @synthesize productId=_productId;
@property(retain, nonatomic) NSDictionary *extInfo; // @synthesize extInfo=_extInfo;
@property(nonatomic) _Bool cookieReWrite; // @synthesize cookieReWrite=_cookieReWrite;
@property(nonatomic) _Bool enableCrypt; // @synthesize enableCrypt=_enableCrypt;
@property(retain, nonatomic) id <DTRpcPrivateEncryptProtocol> privateEncrypt; // @synthesize privateEncrypt=_privateEncrypt;
@property(nonatomic) long long logLevel; // @synthesize logLevel=_logLevel;
@property(nonatomic) _Bool isCrypt; // @synthesize isCrypt=_isCrypt;
@property(nonatomic) _Bool httpGET; // @synthesize httpGET=_httpGET;
@property(nonatomic) _Bool isLoginRPC; // @synthesize isLoginRPC=_isLoginRPC;
@property(nonatomic) _Bool isNSData; // @synthesize isNSData=_isNSData;
@property(nonatomic) _Bool isSpecialChannelRPC; // @synthesize isSpecialChannelRPC=_isSpecialChannelRPC;
@property(nonatomic) _Bool isIgnoreNetStatus; // @synthesize isIgnoreNetStatus=_isIgnoreNetStatus;
@property(nonatomic) double timeoutInterval; // @synthesize timeoutInterval=_timeoutInterval;
@property(nonatomic) _Bool retryable; // @synthesize retryable=_retryable;
@property(nonatomic) long long cacheStrategy; // @synthesize cacheStrategy=_cacheStrategy;
@property(retain, nonatomic) NSString *apdid; // @synthesize apdid=_apdid;
@property(nonatomic) _Bool isAliUserGWMethod; // @synthesize isAliUserGWMethod=_isAliUserGWMethod;
@property(nonatomic) _Bool signCheck; // @synthesize signCheck=_signCheck;
@property(nonatomic) _Bool checkLogin; // @synthesize checkLogin=_checkLogin;
@property(nonatomic) Class elementClass; // @synthesize elementClass=_elementClass;
@property(copy, nonatomic) NSString *returnType; // @synthesize returnType=_returnType;
@property(nonatomic) Class resultClass; // @synthesize resultClass=_resultClass;
@property(copy, nonatomic) NSString *operationType; // @synthesize operationType=_operationType;
- (void).cxx_destruct;
- (id)init;

@end

