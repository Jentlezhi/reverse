//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "GPBMessage.h"

@class NSString;

@interface RoomInfo : GPBMessage
{
}

+ (id)descriptor;

// Remaining properties
@property(nonatomic) unsigned int creatTime; // @dynamic creatTime;
@property(nonatomic) _Bool hasCreatTime; // @dynamic hasCreatTime;
@property(nonatomic) _Bool hasMemCnt; // @dynamic hasMemCnt;
@property(nonatomic) _Bool hasName; // @dynamic hasName;
@property(nonatomic) _Bool hasRoomId; // @dynamic hasRoomId;
@property(nonatomic) _Bool hasUserId; // @dynamic hasUserId;
@property(nonatomic) unsigned int memCnt; // @dynamic memCnt;
@property(copy, nonatomic) NSString *name; // @dynamic name;
@property(nonatomic) unsigned int roomId; // @dynamic roomId;
@property(nonatomic) unsigned long long userId; // @dynamic userId;

@end

